...
class CategoryMonthlyTrendCalculator {
  const CategoryMonthlyTrendCalculator._();

  static MonthlySpendingTrendSeries build({
    required StatisticsPeriod period,
    required List<({DateTime monthStart, double amount})> monthlyRecords,
    ...
    for (final record in monthlyRecords) {
      final key = _monthKey(record.monthStart);
      final bucket = totals[key];
      if (bucket == null) {
        continue;
      }

      totals[key] = (bucket + record.amount).withPrecision(2);
    }
